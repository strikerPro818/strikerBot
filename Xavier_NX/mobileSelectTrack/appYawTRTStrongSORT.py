...
tracker = StrongSORT(model_weights=Path(model_weights), device='cuda', fp16=fp16,max_dist=cfg.STRONGSORT.MAX_DIST,
...
def gen_frames():
    ...
    while True:
        ...
        person_detections = detections[(detections[:, 0] >= 0) & (detections[:, 1] >= 0) &
                                       (detections[:, 2] <= img_width) & (detections[:, 3] <= img_height) &
                                       (detections[:, 5] == 0)]

        outputs = tracker.update(person_detections, img_rgb)
